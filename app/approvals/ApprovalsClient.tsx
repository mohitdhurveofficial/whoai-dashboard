...
const RiskBadge = ({ level }: { level: string }) => {
  ...
  const appliedStyle = styles[level] || "bg-gray-50 text-gray-600 ring-gray-200";

  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-black uppercase tracking-wider ring-1 ring-inset ${appliedStyle}`}
    >
...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: Record<string, string> = {
    pending: "bg-orange-50 text-orange-600 ring-orange-200/50",
    approved: "bg-emerald-50 text-emerald-600 ring-emerald-200/50",
    rejected: "bg-red-50 text-red-600 ring-red-200/50",
  };
  const appliedStyle = styles[status] || "bg-gray-50 text-gray-600 ring-gray-200/50";

  return (
    <span
      className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-black uppercase tracking-wider ring-1 ring-inset ${appliedStyle}`}
    >
      {status}
    </span>
  );
...
const KPICard = ({ title, value, icon: Icon, colorClass, bgClass }: any) => (
  <motion.div
    ...
    className="premium-panel glass p-6 rounded-3xl flex items-center gap-5 transition-transform hover:-translate-y-1"
  >
    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${bgClass} ${colorClass}`}>
      <Icon size={24} strokeWidth={2.5} />
    </div>
    <div>
      <p className="text-[12px] font-bold uppercase tracking-widest text-slate-500">{title}</p>
...
export default function ApprovalsClient({ initialApprovals }: { initialApprovals: ApprovalData[] }) {
  ...
  const handleAction = async (newStatus: "approved" | "rejected") => {
    ...
    try {
      ...
      if (typeof selectedApproval.id === 'number') {
         await updateApproval(selectedApproval.id, newStatus);
      }
    ...
    <div className="flex-1 w-full min-h-screen pb-20">
      <main className="p-6 md:p-10 max-w-[1440px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <KPICard title="Pending Approvals" value={pendingCount} icon={FileClock} colorClass="text-orange-600" bgClass="bg-orange-100/50" />
          <KPICard title="Approved Today" value={approvedCount} icon={CheckCircle2} colorClass="text-emerald-600" bgClass="bg-emerald-100/50" />
          <KPICard title="Rejected Today" value={rejectedCount} icon={XCircle} colorClass="text-red-600" bgClass="bg-red-100/50" />
          <KPICard title="Avg Review Time" value={avgTime} icon={Clock} colorClass="text-blue-600" bgClass="bg-blue-100/50" />
        </div>
                ...
                <div>
                  ...
                  <div className="bg-white p-5 rounded-2xl ring-1 ring-slate-100 shadow-sm space-y-5">
                    <div className="flex justify-between items-center py-1">
                      ...
                      <RiskBadge level={selectedApproval.risk_level} />
                    </div>
